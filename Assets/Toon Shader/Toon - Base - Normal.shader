Shader "Custom/Toon - Base - Normal"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				fixed ndotl = dot(tex2D(_NormalMap, i.uv), _WorldSpaceLightPos0.xyz) * attenuation;
		...
		Pass
		{
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				fixed ndotl = dot(tex2D(_NormalMap, i.uv), lightDir) * attenuation;
