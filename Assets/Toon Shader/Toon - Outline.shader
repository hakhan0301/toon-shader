Shader "Custom/Toon - Outline"
{
	Properties
	{
		...
		_OutlineAmount("Outline Amount", Float) = 1
	...
	SubShader
	{
		...
		Pass
		{
			...
			sampler2D _ToonLUT;
			fixed4 _Color;
			fixed _NormalValueScale;
			float _NormalValuePower;
		...
		Pass
		{
			...
			v2f vert(appdata v)
			{
				...
				v.vertex.xyz += (v.normal) * _OutlineAmount * .001;
				o.pos = UnityObjectToClipPos(v.vertex);
				o.uv = v.uv;
			...
			fixed4 frag(v2f i) : SV_Target
			{
				return tex2D(_MainTex, i.uv) * _OutlineColor;
			}
			ENDCG
		}
